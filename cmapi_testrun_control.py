    ...
    result = run_check_attempt(
        ...
        script_text=render_dde_execute_script(
            ...
            [
                ...
                '    set wno 0',
                '}',
                '# --- Set view size only if dimensions differ (skip avoids C++ ConfigFBO) ---',
                ...
                '}',
                '# Read back from View dict (avoids update / GL widget race with render timer)',
                'format "width=%s;height=%s;widget=%s" [dict get $View($wno) Width] [dict get $View($wno) Height] $wpath',
            ],
        ),
