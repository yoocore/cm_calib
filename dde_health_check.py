    ...
    return [
        ...
        (
            ...
            render_dde_execute_script(
                ...
                "IPG-MOVIE",
                [
                    r"set vno $::View(ev.view)",
                    r"set win_state [wm state .]",
                    r"if {$win_state eq {iconic}} {",
                    r"    # Window is iconic (minimized) - FBO should be available; test it",
                    r"    set fbo [FBO new 16 16 -tex rgb -noclear]",
                    r"    if {[catch {FBO begin $fbo} err]} {",
                    r"        catch {FBO delete $fbo}",
                    r"        error {FBO probe failed (begin): $err}",
                    r"    }",
                    r"    catch {FBO end}",
                    r"    catch {FBO delete $fbo}",
                    r"    list ok {FBO probe passed (iconic)}",
                    r"} elseif {$win_state eq {normal}} {",
                    r"    # Window is normal - capture uses PrintWindow, not FBO",
                    r"    # Skip FBO test; just verify rendering responds",
                    r"    list ok {FBO probe skipped (normal mode)}",
                    r"} else {",
                    r"    # Unknown window state (withdrawn/zoomed) - probe as normal",
                    r"    list ok {FBO probe skipped ($win_state)}",
                    r"}",
                ],
            ),
        ),
