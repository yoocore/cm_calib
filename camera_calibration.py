        ...
        for attempt in range(attempt_count):
            ...
            script_text = render_dde_execute_script(
                ...
                [
                    ...
                    '# Temporarily restore window if minimized (GL context suspended when iconic)',
                    'set __was_iconic 0',
                    'if {![catch {set __state [wm state .]}] && $__state eq "iconic"} {',
                    '    set __was_iconic 1',
                    '    wm state . normal',
                    '    update',
                    '    update idletasks',
                    '}',
                    'set __capture_ok [catch {',
                    '    UpdateView $vno',
                    ...
                    '    set __capture_ok 0',
                    '} __capture_err]',
                    '# Restore minimized state if window was iconic before',
                    'if {$__was_iconic} { wm state . iconic; update }',
                    'if {$__capture_ok} { error $__capture_err }',
                ],
            )
            script_path.write_text(script_text, encoding="utf-8")
            _unlink_if_exists(result_path)
