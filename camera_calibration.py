        ...
        for attempt in range(attempt_count):
            ...
            script_text = render_dde_execute_script(
                ...
                [
                    ...
                    "    update",
"} finally {",
                    "    # Restore original over no-op: delete no-op first, then restore.",
                    "    # Tcl rename does NOT overwrite an existing command, so we must delete",
                    "    # the no-op before renaming the saved original back into place.",
