        ...
        for attempt in range(attempt_count):
            ...
            script_text = render_dde_execute_script(
                ...
                [
                    ...
                    "# --- Guard: skip height bump if view already has valid dimensions ---",
                    "# IPG-MOVIE has a C++-level Configure binding (On_Configure) that calls ConfigFBO",
                    "# directly. The Tcl-level UpdateView_TimerProc rename defense cannot protect against",
                    "# this. Unnecessary height bumps (View::SetSize H+1 then H) trigger two Configure",
                    "# events -> two ConfigFBO calls -> FBO corruption.",
                    "# See: bind .view0.gl0 <Configure> = EventCallbacks::GUI::Window::On_Configure %W",
                    "if {$vp_w > 0 && $vp_h > 0} {",
                    "    puts $__copilot_remote_out \"DIAG_SKIP_BUMP: view already $vp_w x$vp_h\"",
                    "} else {",
                    ...
                    "    }",
                    "    # --- Defense: cancel UpdateView_TimerProc before processing events ---",
                    "    try {",
                    ...
                    "    }",
                    "}",
                    "# --- Ensure window visible for Win32 capture (FBO path unreliable due to C++ ConfigFBO conflicts) ---",
