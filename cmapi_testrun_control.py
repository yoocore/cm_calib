    ...
    result = run_check_attempt(
        ...
        script_text=render_dde_execute_script(
            ...
            [
                ...
                '    set wno 0',
                '}',
                '# --- Guard: skip height bump if view already has target dimensions ---',
                '# Prevents C++-level Configure -> ConfigFBO corruption (Tcl rename cannot protect).',
                'set __cur_w [$wpath.gl0 cget -width]',
                'set __cur_h [$wpath.gl0 cget -height]',
                'puts "DIAG_ENSURE_VP: cur=$__cur_w x$__cur_h"',
                f'if {{$__cur_w == {target_width} && $__cur_h == {target_height}}} {{',
                '    puts "DIAG_SKIP_ENSURE_BUMP: view already $__cur_w x$__cur_h"',
                '} else {',
                ...
                '    # --- Sync the Tcl View() dict so CheckViewPort sees no mismatch ---',
                ...
                '# defense against any remaining or re-registered timers that call ConfigFBO.',
                'try {',
                '    catch {after cancel UpdateView_TimerProc}',
                ...
                '    update',
                '} finally {',
                '    # Restore original UpdateView_TimerProc: delete no-op first, then restore.',
                '    # Tcl rename does NOT overwrite an existing command, so we must delete the',
                '    # no-op before renaming the saved original back into place.',
                '    catch {rename UpdateView_TimerProc {}}',
                ...
                'catch {trace add command CheckViewPort delete ::OnCheckViewPortDelete}',
                '::ReGuardCheckViewPort',
                '# --- Re-schedule the rendering loop timer AFTER event processing (not before — would trigger ConfigFBO in unstable GL context) ---',
                'catch {after 0 UpdateView_TimerProc}',
                'update',
