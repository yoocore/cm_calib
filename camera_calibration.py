...
class CameraCalibrator:
    ...
    def _capture_movie_via_dde_fbo(self, tag: str) -> Path:
        ...
        for attempt in range(attempt_count):
            ...
            # Use real image dimensions for FBO — NOT the viewport widget size,
            # which may still reflect the previous camera's aspect ratio.
            ref_h, ref_w = self.real_img.shape[:2]
            script_text = render_dde_execute_script(
                result_path,
                "IPG-MOVIE",
                [
                    ...
                    "}",
                    "# --- shared FBO: use real-image dims, rebuild on size change ---",
                    "if {![info exists ::calib_fbo] || $::calib_fbo eq \"\"} {",
                    "    set ::calib_fbo [FBO new $ref_w $ref_h -tex rgb -noclear]",
                    "} elseif {$ref_w != $::calib_fbo_w || $ref_h != $::calib_fbo_h} {",
                    "    catch {FBO end}",
                    "    catch {FBO delete $::calib_fbo}",
                    "    set ::calib_fbo [FBO new $ref_w $ref_h -tex rgb -noclear]",
                    "}",
                    "set ::calib_fbo_w $ref_w; set ::calib_fbo_h $ref_h",
                    "set update_rc [catch {",
                    "    FBO begin $::calib_fbo",
                    "    UpdateView $vno",
                    "    FBO end",
