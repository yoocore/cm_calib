...
def _default_output_name_from_config(config_path: Optional[Path]) -> str:
    if config_path is not None:
        ...
        prefix = "camera_rpa_config."
        if name.startswith(prefix):
            name = name[len(prefix):]
        name = name.replace(".", "_").strip("_")
        if name:
            return name
    return "camera_rpa_run"
...
class RPACalibrator:
    ...
    def __init__(self, cfg: dict):
        ...
        default_runtime_path = calibration_root / "copilot_script_control_runtime.tcl"
        default_command_path = calibration_root / "script_control_camera_tree_probe.tcl"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="IPGMovie settings RPA multi-board matching loop"
    )
    parser.add_argument(
        "--config",
        required=True,
        help="Path to JSON config, for example camera_rpa_config.rear_tv.json",
    )
    parser.add_argument(
        "--inspect-controls",
        action="store_true",
